...
over sequences of annotated values, implicitly by means
of program execution.
Imagine an interpreter that runs the deterministic parts of a program 
...
In addition to this execution, the interpreter also keeps a log that records
...
A \textit{trace} $\pmb{x}$ is a sequence of triples $(F,x,\alpha)$. It is feasible if
it is precisely the triple part of the log of some execution. 
Such a feasible trace records all the random choices made during execution,
and uniquely determines the rest of the execution and its log, because all the non-sample
forms of Anglican programs are deterministic. In particular, each of these feasible traces
decides a sequence of pairs $(G_j, y_j)$ for observed values.
We call the sequence \textit{image of $\pmb{x}$} and denote it by~$\pmb{y}$. 

A (almost surely terminating) probabilistic program defines a probability distribution 
over feasible traces $\pmb{x}$, which has the probability density 
...
\begin{align}
  ...
  \label{eqn:p-trace}
\end{align}
for the image $\pmb{y}$ of $\pmb{x}$, and $Z$ is the normalization constant $Z := \int \gamma(\pmb{x})d(\pmb{x})$

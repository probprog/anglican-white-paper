...
\begin{table}
    ...
    \begin{tabular}{r|r|r|r|r}
        ...
        Clojure &  0.32 & 1.45 & 38.1  &  1245. \\ \hline
...
Table~\ref{tbl:webppl} shows results for two models from
\href{https://github.com/probmods/webppl}{WebPPL
distribution}, \textit{LDA} and
\textit{Logistic regression}. For
comparison, we re-implemented the models in Anglican, as close
as possible to the WebPPL implementations, and run the same
inference algorithm (called MCMC in WebPPL, LMH in Anglican)
with the same number of iterations. Anglican outperforms WebPPL
on both models. This can be attributed both to a better
transformed representation of probabilistic programs and to more
efficient architecture of the inference engine.
\begin{table}
    \centering
    \begin{tabular}{r|c|c}
        {\bf model}  & {\bf LDA} & {\bf Logistic regression} \\ \hline
        Anglican & 22. & 4.8 \\ \hline
        WebPPL & 31. & 9.4
    \end{tabular}
    ...
    100 runs. Anglican outperforms WebPPL on both models.}
    \label{tbl:webppl}
\end{table}

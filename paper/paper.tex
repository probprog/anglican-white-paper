...
(defquery deli [time-to-arrive-prior
 ...
 (let [is-same-customer (sample (flip p-same))
       observe-customer (if is-same-customer
                           same-customer
                           different-customer)]
    (predict :is-same-customer is-same-customer)
    (observe-customer time-to-arrive-prior
                      lunch-delay
                      dinner-delay)))
\end{lstlisting}

Performing inference on query \texttt{deli} predicts the
...
\begin{lstlisting}[style=default]
(def p-same+ 
     (/ (count (filter :same-customer predicts))
        (double N)))
\end{lstlisting}
...
predicted values in retrieved samples to approximate the
...
inferences approximates the posterior by known distributions,

...
\texttt{observe} in an algorithm-specific manner, as well
as to construct an appropriate result on the 
...
other forms of Clojure may be in the future but are not now. 
...
Clojure provides special constructs \texttt{loop} and \texttt{recur}
for writing tail-recursive programs. Such constructs are not necessary
in Anglican, because Anglican programs are CPS-converted and do not
use stacks; no recursive call in Anglican can lead to stack overflow.
In Anglican, recursive calls to functions should be used instead of

...
\hy{I found this section confusing in the following sense. First,
it does not highlight the most interesting aspects of the compilation
of Anglican. It gives a good overview on how the compilation is
implemented, but a reader might want to see something unique and
interesting about Anglican. To me, the most interesting part of the
compilation is the treatment of probabilistic constructs in Anglican's
CPS transformation. How are sample,
observe and stochastic memoization compiled? How should the standard
CPS transformation be modified in order to handle these constructs?
The use of state can be one answer for the second question.
The second most interesting part is about the use of trampolining
the mix with Clojure code.
Second, the section assumes that a reader is already familiar 
with Clojure or something specific to probabilistic programming. For 
instance, it might be that a reader  has never heard about
Clojure records and stochastic memoization. Then, she or he
would have difficulties in following this section. 
What should we do? One option is not to do anything. Another is
to take care of the second point. That is, we make the section
easily followed by someone who does not know Clojure. The third
is to restructure the section and to take care of my first criticism.}

\dt{I believe I partially addressed the above comment, but need to
think more about it.}
...
An inference algorithm is an implementation of the (ad-hoc) 
polymorphic function \texttt{infer} declared as Clojure's multimethod
in the \texttt{anglican.inference} namespace. The
function accepts an algorithm identifier, a query --- the
...
\hy{This subsection is also not quite self-contained. Someone
who is not familiar with Clojure or Anglican codebase is likely
to get lost. On the other hand, it is a good overview or guide
for those who want to study the implementation of Anglican. One
way to address this presentation issue while keeping the benefits
of the current text is to include some extended examples. For instance,
in the beginning of the paper, we might include several
Anglican examples. Then, we show that how these examples get
cps-transformed. Next, we present the implementation of inference
algorithms in Anglican using semi pseudo code; this is not quite
an implementation in Clojure but gives a good sketch on how
various functions are implemented in each inference algorithm.
Finally, we show how these algorithms work on cps-transformed 
examples.}
...
Checkpoints can be uniquely named at the compilation time;
...
\begin{enumerate}
    \item The address size is linear in stack depth.
...
re-use of previously drawn values, produces addresses of constant size,
and allows manual computation of checkpoint addresses at runtime
when the default automatic scheme is insufficient. 


\begin{verbatim}
A simple heuristic gives the same acceptance rate as with interpreted Anglican on all examples,
including Neil’s Locomotion, as follows:

  a) each sample form has a unique id, generated at compile time. It does anyway, and this 
      has almost zero cost.
  b) address is a pair 
        [id-of-sample-form number-of-previous-occurences]
      the occurences are of the same form.
  c) if a sequence of random choices with the same id is interrupted, the count is padded up 
  by a small power of 2, for example 16.

For example, if random choices
...
are encountered in the trace, then the addresses are:
...
This way, every occurence of random choice has a unique address, but small disturbances (like
removal or addition of a single choice of a particular kind) do not derail the entire sequence.
This aligns well with recursive patterns used in DPmem, for example.

I have some thoughts about stopping early in addition to starting late, but will do it when there
are less deadlines. Right now, the code runs MUCH MUCH faster, it should take roughly 10
hours to produce the same amount of data for Neil’s Locomotion example which was produced
during 14 DAYS of the holidays. 
\end{verbatim}

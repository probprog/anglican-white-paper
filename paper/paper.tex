...
as well as from Anglican test suite. For the comparison, we
re-implemented WebPPL models in Anglican, and Anglican models in
WebPPL, following as close as possible the original
implementations, and run the same inference algorithm (called
MCMC in WebPPL, LMH in Anglican) with the same number of
iterations. Anglican outperforms WebPPL on all models. This can
be attributed both to a better transformed representation of
probabilistic programs and to more efficient architecture of the
inference engine.
\begin{table}
    \centering
	\begin{tabular}{r|S[table-format=2.2]|S[table-format=2.2]}
		...
		LDA & 22. & 31. \\ 
		\shortstack{Linear regression}  & 0. & 0. \\
		\shortstack{Logistic regression} & 6.2 & 9.4 \\
		\shortstack{Multivariate regression} & 0. & 0. \\
		\shortstack{Simple branching} & 0. & 0.
    \end{tabular}

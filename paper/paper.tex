...
We call the triple part of a log \textit{trace}, and denote this sequence of $(F,x,\alpha)$'s 
by $\pmb{x}$. This part records all the random 
choices of an execution. Thus, if the trace of a log is fixed, the rest of the log 
is decided determistically by the program. A trace $\pmb{x}$ induces
a sequence of pairs $(G_j, y_j)$ of distributions and values of
\textit{observed} random variables. We call this sequence \textit{image of $\pmb{x}$} and
denote it by $\pmb{y}$. A subset of random choices in $\pmb{x}$ forms the result of the program.
We denote this result (i.e. the subsequence of $\pmb{x}$ with these choices) by $\pmb{z}$. 
%For notational simplicity we assume that the program
...
%that probabilistic programs define distributions over traces. Also, $\vec{x}$ is not defined.}
...
choices $\pmb{x}$ and the likelihood of all observations in the image $\pmb{y}$ 
of $\pmb{x}$:
\begin{align}
  \gamma(\pmb{x}) 
  &:= 
  ...
  \label{eqn:p-trace}
\end{align}
This density induces the distribution of the results $\pmb{z}$ of the program
by marginalization. The objective of probabilistic inference 
is to characterize this distribution of $\pmb{z}$.
